...
One script reads two sheets from Supp_Table1.xlsx. Step 1 (High_Kac): single-protein filter, K-only filter, max_loc_prob greater than 0.99 identifies Positives. Step 2 (High_pSTY): uses terminal-K prefix keys built from positives to extract K residues in phospho-peptides as Negatives.
...
Dedup collapses 56,129 rows to 35,307 unique sites - this is the unfiltered dataset. Then two-stage K-position balancing: Stage 1 downsamples Negatives until K_first percentage matches Positives. Stage 2 downsamples Positives until K_second_last matches.
